 ...
 * 표에 표시할 커플명.
 * [행 멤버][열 멤버] 순서.
 * 대각선(본인조합)을 포함해 각 멤버 이니셜(융·슈·솜·힝·롱)을
 * 그대로 이어붙인 기본값이니, 원하는 조합명으로 자유롭게 바꿔서 쓰시면 돼요.
 */
const pairNames = [
    ["융융", "융슈", "융솜", "융힝", "융롱"],
    ["슈융", "슈슈", "슈솜", "슈힝", "슈롱"],
...
const CUSTOM_COLOR_KEY = "illit-custom-colors";
...
const STORAGE_KEY = "illit-ritpes-rps";
const LR_STORAGE_KEY = "illit-lr-rps";
const LR_CELL_COUNT = 12;

/* 행/열 개별 숨기기 상태 (멤버 인덱스 기준, rows/cols 따로 관리) */
const HIDDEN_KEY = "illit-hidden-members";
...
const SELF_PAIR_KEY = "illit-include-selfpair";
...
saveBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        const fileLabel = currentTab === "rps" ? "릿페스_취향표" : "공수_취향표";

        const link = document.createElement("a");
        link.href = currentBlobUrl;
        link.download = `ILLIT_${fileLabel}.png`;
